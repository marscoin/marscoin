...
static void UpdateBlockAvailability(NodeId nodeid, const uint256 &hash) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (pindex && pindex->nChainWork > 0) {
        //LogPrintf("An actually better block was announced.\n");
        if (state->pindexBestKnownBlock == nullptr || pindex->nChainWork >= state->pindexBestKnownBlock->nChainWork) {
            state->pindexBestKnownBlock = pindex;
        }
    } else {
        //LogPrintf("An unknown block was announced; just assume that the latest one is the best one.\n");
...
{        
    ...
    //LogPrintf("Checking how many blocks to download.. from Node: %s window size: %i\n", state->name, count);

    // Make sure pindexBestKnownBlock is up to date, we'll need it.
    ProcessBlockAvailability(nodeid);
    
    if(state->pindexBestKnownBlock != nullptr){
        //LogPrintf(" bestknownblockchainwork: %i \n", state->pindexBestKnownBlock->nChainWork.GetCompact());
        //LogPrintf("and activeTip: %i \n", chainActive.Tip()->nChainWork.GetCompact());
        //LogPrintf("or  nMinimumChainWork: %i\n", nMinimumChainWork.GetCompact());
    }
    ...
    //LogPrintf("Interesting peer...\n"); 
    if (state->pindexBestKnownBlock == nullptr)
    {
        //LogPrintf("...yet waiting for pindexBestKnownBlock to be set..");
        return;
    }
    ...
    if (state->pindexLastCommonBlock == nullptr) {
        ...
        //LogPrintf("pindexLastCommonBlock failed ...\n");
        state->pindexLastCommonBlock = chainActive[std::min(state->pindexBestKnownBlock->nHeight, chainActive.Height())];
        //LogPrintf("pindexLastCommonBlock continued ...\n");
    }
    //LogPrintf("LastCommonBlock: %i\n", state->pindexLastCommonBlock->nHeight);
    //LogPrintf("Bootstrapped pindexLastCommonBlock ...\n");
    ...
    //LogPrintf("Not reorganized peer ...\n");
...
bool static ProcessHeadersMessage(CNode *pfrom, CConnman *connman, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool punish_duplicate_invalid)
{
    ...
    if (nCount == 0) {
        ...
        LogPrintf("Nothing interesting. Stop asking this peers for more headers.\n");
        return true;
    }
    ...
    const CBlockIndex *pindexLast = nullptr;
    {
        ...
        if (!LookupBlockIndex(headers[0].hashPrevBlock) && nCount < MAX_BLOCKS_TO_ANNOUNCE) {
            ...
            LogPrintf("1Set hashLastUnknownBlock for this peer so it can be used for download later on\n");
...

    {
        ...
        assert(pindexLast);
        //LogPrintf("2Set hashLastUnknownBlock for this peer so it can be used for download later on\n");
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            if (!chainActive.Contains(pindexWalk)) {
                ...
                for (const CBlockIndex *pindex : reverse_iterate(vToFetch)) {
                    ...
                    LogPrint(BCLog::NET, "(1) Requesting block %s from  peer=%d\n",
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    else if (strCommand == NetMsgType::INV)
    {
        ...
        for (CInv &inv : vInv)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                //LogPrintf("3Set hashLastUnknownBlock for this peer so it can be used for download later on\n");
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        assert(pindex);
        LogPrintf("4Set hashLastUnknownBlock for this peer so it can be used for download later on\n");
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!pto->fClient && ((fFetch && !pto->m_limited_node) || !IsInitialBlockDownload()) && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            //LogPrintf("Downloading...\n");
            //LogPrintf("To be downloaded: %i\n", vToDownload.size());
            for (const CBlockIndex *pindex : vToDownload) {
                uint32_t nFetchFlags = GetFetchFlags(pto);
                vGetData.push_back(CInv(MSG_BLOCK | nFetchFlags, pindex->GetBlockHash()));
                MarkBlockAsInFlight(pto->GetId(), pindex->GetBlockHash(), pindex);
                LogPrint(BCLog::NET, "(2) Requesting block %s (%d) peer=%d\n", pindex->GetBlockHash().ToString(),
