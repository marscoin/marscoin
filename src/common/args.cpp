...
const char * const BITCOIN_CONF_FILENAME = "marscoin.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Marscoin
    //   new: C:\Users\Username\AppData\Local\Marscoin
    // macOS: ~/Library/Application Support/Marscoin
    // Unix-like: ~/.marscoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Marscoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Marscoin";
#else
    fs::path pathRet;
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    ...
    const bool fQDevNet = get_net("-qdevnet");
    const bool fSigNet  = get_net("-signet");
    const bool fTestNet = get_net("-testnet");
    const bool fTestNet4 = get_net("-testnet4");
    const auto chain_arg = GetArg("-chain");

    if ((int)chain_arg.has_value() + (int)fRegTest + (int)fQDevNet + (int)fSigNet + (int)fTestNet + (int)fTestNet4 > 1) {
        throw std::runtime_error("Invalid combination of -regtest, -qdevnet, -signet, -testnet, -testnet4 and -chain. Can use at most one.");
    }
    if (chain_arg) {
        if (*chain_arg == "qdevnet") return ChainType::REGTEST;
