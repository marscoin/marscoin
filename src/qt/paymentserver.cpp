...
const int MARSCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString MARSCOIN_IPC_PREFIX("marscoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(MARSCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(MARSCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(MARSCOIN_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click marscoin: links)
...
{
    // clicking on marscoin: URLs creates FileOpen events on the Mac:
