...
bool CheckProofOfWork(const CBlockHeader& block, const Consensus::Params& params)
{
    ...
    {
        if (block.IsAuxpow()) {
            LogError("%s : no auxpow on block with auxpow version\n", __func__);
            return false;
        }
    ...
    if (!block.IsAuxpow()) {
        LogError("%s : auxpow on block with non-auxpow version\n", __func__);
        return false;
    }

    if (block.auxpow->getParentBlock().IsAuxpow()) {
        LogError("%s : auxpow parent block has auxpow version\n", __func__);
        return false;
    }
    ...
    if (!block.auxpow->check(block.GetHash(), block.GetChainId(), params)) {
        LogError("%s : AUX POW is not valid\n", __func__);
        return false;
    }
